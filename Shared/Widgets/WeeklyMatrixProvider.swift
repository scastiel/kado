...
struct WeeklyMatrixProvider: TimelineProvider {
    func placeholder(in context: Context) -> WeeklyMatrixEntry {
        .placeholder()
    }

    func getSnapshot(in context: Context, completion: @escaping @Sendable (WeeklyMatrixEntry) -> Void) {
        Task { @MainActor in
            completion((try? Self.buildEntry(asOf: .now)) ?? .placeholder())
        }
    }

    func getTimeline(in context: Context, completion: @escaping @Sendable (Timeline<WeeklyMatrixEntry>) -> Void) {
        Task { @MainActor in
            let now = Date.now
