...
### The capped picker and the de-duplicated picker are two different pickers

- **What happened**: `@Parameter(size:)` — the first attempt's fix for "the sheet let you pick eight for a tile of five" — makes the sheet a *list editor*: capped, drag-to-reorder, and able to take the same habit twice because every "Add New Item" offers the full list again. The author hit that within minutes. Without `size:` the sheet is a *checklist*: no duplicates, tap order kept, no cap. Nothing combines the two: `size:` and `optionsProvider:` are separate initializers; an options provider behind a checklist must return everything or the checked rows disappear; and an `IntentParameterDependency` on the entity's own query — the natural way to leave out what's already picked — is a cycle AppIntents does not detect. The extension looped on "Building resolver for parameter habits" until it was killed, and its log filled the disk.
- **What we did**: chose the checklist, knowingly, and wrote the trade into `SelectHabitsIntent`. `WidgetIntentManifestTests` now pins the *absence* of a cap. The per-family intents built to host a capping options provider were reverted before commit.
- **Lesson**: **the parameter's declaration is a UI decision, not a validation one.** `size:` doesn't add a rule to a picker; it swaps the picker. Read what each shape does on the device before promising what it enforces — and never let a query depend on the parameter it resolves.
