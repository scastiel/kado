...
>
> - **Task 0** — a debug `Logger` in each `AppIntentTimelineProvider` (`family= picked= paramNil=`), and `Scripts/resign-simulator.sh` + a `make run` hook so AppIntents can be verified on iOS 26.x simulators.
...
> - **Reversed after first use, too**: the picker is a checklist, not the capped list editor `@Parameter(size:)` produces. The list editor enforced 5 / 8 / 5 and could reorder by drag, but it let the same habit be added twice, each copy spending a slot. A checklist cannot hold duplicates and keeps tap order; it cannot be capped either — the two are different initializers, and every route to a capped, de-duplicated picker was tried and closed (see compound). The tile draws the first N picks; "Pick up to N." sits above the checklist in the sheet.
